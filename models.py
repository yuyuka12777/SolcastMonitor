...
class SolarForecast:
    """太陽光予測データを格納するクラス"""
    time: datetime
    cloud_opacity: float  # 雲の不透明度 (0-1)
    wind_speed: float  # 10m高さでの風速 (m/s)
    wind_direction: float  # 10m高さでの風向 (度、0=北, 90=東, 180=南, 270=西)
    zenith: float  # 太陽天頂角 (度、0=真上、90=地平線)
    azimuth: float  # 太陽方位角 (度、0=北, 90=東, 180=南, 270=西)
    ghi: float  # 全天日射量-水平面 (W/m^2)
    gti: float  # 全天傾斜日射量-傾斜面 (W/m^2)
    forecast_radiation: float  # 直達日射量 (W/m^2)
    gti_valid: bool  # GTIが有効かどうか
